...
from cnn_image_processing.filters.writers import ImageWriter
...
class FCN(multiprocessing.Process):
    ...
    def read(self, queue, in_packets, parts_meta):
        """
        Read the incoming packets from the in_queue
        """
        i_patch = 0
        i_batch = 0
    ...
    def gather_parts(self, fcn, in_packets, parts_meta, batch_size):
        ...
        for i_batch in xrange(batch_size):
            ...
            for key in pop_keys:
                if len(parts_meta[key]) > 0:
                    parts_meta[key].pop(0)
            ...
            if part_meta['part_id'] == (part_meta['n_parts'] - 1):
                ...
                for key in pop_keys:
                    if len(in_packets[key]) > 0:
                        in_packets[key].pop(0)
